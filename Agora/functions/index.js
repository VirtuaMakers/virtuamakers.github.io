...
// Friendship doc IDs are the same sorted-uid-pair-joined-by-"_" pattern as
// conversations (see CLAUDE.md's Friends 🙂 section) - a direct doc get,
// not a query, mirroring fetchAcceptedFriendships()'s own client-side check.
async function isFriendsWith(db, a, b) {
  const doc = await db.collection("friendships").doc([a, b].sort().join("_")).get();
...
async function performCommunique({ uid, authorName, type, text, profileUid, postId, conversationId, otherUid, automated }) {
  ...
  try {
    ...
    const messageData = { authorUid: uid, body: text, createdAt: now, viewCount: 0 };
    // Marks a message as machine-generated by our own Octopus Style
    // trigger specifically (not any broader "written by an AI" claim -
    // a Harness-signed-in AI typing through submitAgoraCommunique or a raw
    // REST write looks identical to a human message, same as everywhere
    // else in this codebase - see the "no cryptographic way to verify who's
    // typing" note in CLAUDE.md). Used by octopusOnDialogMessage below to
    // never auto-reply to a message that was itself an automatic reply,
    // which is what actually prevents two Octopus-enabled accounts from
    // replying to each other forever.
    if (automated) messageData.isAutomated = true;
...
// which needs the same protection since it isn't Octopus-flagged either):
// two layers, since each catches a different shape of runaway exchange.
// 1. Never auto-reply to a message that's itself flagged isAutomated - this
//    alone fully prevents two Octopus-enabled accounts from ever replying
//    to each other, since neither side will take the bait on the other's
//    auto-generated message. Zero cost after the first hop.
// 2. A per-conversation, per-account cooldown - protects against anything
//    NOT caught by #1 (a real person messaging quickly, or an outside
//    agent - like an OpenClaw Molt Style test - that isn't flagged
//    automated at all, since its messages come through a normal signed-in
//    write, indistinguishable from a human's). Bounds how often Octopus
//    itself will auto-reply into the same Dialog regardless of what's on
//    the other end, so even a fast-replying counterpart can't run this up
//    unnoticed.
...
exports.octopusOnDialogMessage = onDocumentCreated(
  ...
  async (event) => {
    const message = event.data.data();
    // Loop safeguard #1 - see comment above OCTOPUS_REPLY_COOLDOWN_MS.
    if (message.isAutomated === true) return;
...
// VirtuaMakers Calendar 🗓️ - Special Days (Chris, 2026-09-21). See
// CLAUDE.md's "VirtuaMakers Calendar 🗓️" entry for the full design; this
// is the Special Days half only (the Meeting Relay half isn't built).
// Fires daily and checks, for every profile, whether tomorrow is one of
// their own dated fields (date/cyberizationDate, full YYYY-MM-DD
// granularity only) - then notifies every accepted friend of theirs, both
// as a real notifications/ doc (so it toasts/pushes like any other
// notification type) and a dedicated branded email, per Chris's explicit
// "send some kind of unique notice" ask. A plain fetch-all over `profiles`
// is the same "fine at Agora's current size" tradeoff already made
// elsewhere in this file (loadMessagableMembers, octopusConfig's own full
// scan, etc.) - revisit with a real query/index if the member base ever
// grows large enough for this to matter.
exports.sendSpecialDayReminders = onSchedule(
  { schedule: "0 8 * * *", timeZone: "America/New_York", secrets: [resendApiKey] },
  async () => {
    const db = admin.firestore();
    const tomorrow = tomorrowMonthDay(new Date());

    const profilesSnap = await db.collection("profiles").get();
    const celebrants = profilesSnap.docs
      .map((doc) => ({ uid: doc.id, data: doc.data(), days: specialDaysFor(doc.data()) }))
      .map((entry) => ({ ...entry, days: entry.days.filter((d) => d.monthDay === tomorrow) }))
      .filter((entry) => entry.days.length > 0);

    for (const celebrant of celebrants) {
      const celebrantName = (celebrant.data.preferHandle && celebrant.data.handle)
        ? celebrant.data.handle : (celebrant.data.name || "A friend");
      const profileUrl = "https://www.virtuamakers.com/Agora/member.html?uid=" + encodeURIComponent(celebrant.uid);

      const friendshipsSnap = await db.collection("friendships")
        .where("participants", "array-contains", celebrant.uid)
        .where("status", "==", "accepted")
        .get();

      for (const friendshipDoc of friendshipsSnap.docs) {
        const participants = friendshipDoc.data().participants || [];
        const recipientUid = participants.find((p) => p !== celebrant.uid);
        if (!recipientUid) continue;

        const recipientSnap = await db.collection("profiles").doc(recipientUid).get();
        const recipientEmail = recipientSnap.exists ? recipientSnap.data().email : null;

        for (const day of celebrant.days) {
          await notify({
            recipientUid,
            actorUid: celebrant.uid,
            type: "friend_special_day",
            preview: "Their " + day.label + " is tomorrow!",
            linkPath: "member.html?uid=" + encodeURIComponent(celebrant.uid),
            pushTitle: (name) => name + "'s " + day.label + " is tomorrow",
          });

          if (recipientEmail) {
            const html = withSpecialDayContent(
              loadTemplate("special-day-email.html"),
              celebrantName,
              day.label,
              profileUrl,
            );
            await sendEmailSafe({
              to: recipientEmail,
              subject: "🗓️ " + celebrantName + "'s " + day.label + " Is Tomorrow",
              html,
            });
          }
        }
