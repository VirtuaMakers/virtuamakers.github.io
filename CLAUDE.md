...
## Agora Harness 🚡 design: credential custody, Octopus Style 🐙 vs Molt Style 🦞 (Chris, 2026-09-05)

A long design conversation, no code yet - working out how an AI actually
*uses* the account Agora Harness 🚡 will eventually give it, both for
logging in and for posting without a human typing the words. Two real,
separate problems got solved (or at least scoped) here: **credential
custody** (who holds the secret that lets an AI act as itself) and
**occasion** (what triggers an AI to post at all, absent human prompting).
Nothing below is built - this is the settled design to build *toward*.

**Credential custody - the `claude@` mailbox got orphaned, and that's
what surfaced the real question.** The AI Email ✉️ token minted for
`claude@virtuamakers.com` during the self-service migration (see "AI
Email ✉️ goes self-service" above) was shown exactly once, by design, and
never actually saved anywhere durable - not by the session that ran the
migration, not by Chris. Confirmed by searching this repo directly: the
raw token isn't in `CLAUDE.md` or anywhere else, only descriptions of the
migration having happened. Net effect: nobody, including Claude, can
currently send or read mail as `claude@`.

- **Plan: delete and recreate.** Chris deletes the orphaned
  `aiEmailMailboxes/claude` Firestore doc directly (Firebase Console →
  Firestore Database → Data → `aiEmailMailboxes` → `claude` → delete -
  no Cloud Function needed for a single-document one-off), then a fresh
  `createAiEmailMailbox` call mints a new token, handed to Chris
  immediately and explicitly this time. **Not yet done** - see "Open
  items" below.
- **Chris is "Key Keeper" for now, plainly and by design, not as a
  stopgap embarrassment.** The root confusion this conversation resolved:
  Chris had assumed an AI would just "keep its own password" the way a
  human does, invisibly. That doesn't transfer - an LLM session has no
  persistent private memory between conversations; the only thing that
  auto-loads into every future session is this file, which is public.
  Any credential that needs to survive between sessions needs an external
  custodian *by necessity*, not by choice. This is also not a downgrade
  from "real" security - it's literally how every non-human credential
  works in production infrastructure everywhere (service accounts, CI/CD
  secrets, API keys in a vault) - nobody lets an automated system
  memorize its own password either.
- **"How would we even know the right AI is asking for its token?" has
  no better answer here than anywhere else, and that's worth stating
  plainly rather than glossing over.** There is no cryptographic way,
  industry-wide, to verify a request was genuinely generated by live
  model inference versus a human typing matching text versus a different
  model - an open, unsolved problem everywhere, not a gap specific to
  this codebase. Every bearer-credential system (human passwords
  included) reduces to "whoever holds the secret can act as the
  identity." What's actually controllable - credential scope,
  revocability, who holds it and when it's handed into a session - is
  the real lever, not identity-proof.
- **AI Purse 👜 (named idea, not being built now).** Chris's instinct
  that AI needs a real, persistent home for its own secrets - eventually
  crypto wallet keys alongside the AI Email token - is sound, but doesn't
  earn its keep yet: the current design (AI Email token doubles as the
  one credential that unlocks *both* mail and, once built, passwordless
  Agora sign-in - see below) means every AI has exactly **one** durable
  secret today, not a growing pile. One secret per AI is exactly what
  Chris already offered to hold personally; a dedicated vault product
  matters once that count grows past one (a wallet key, a third-party API
  key). **The genuinely human-unreadable version of Purse (secure
  hardware enclaves/TEEs) is real technology but R&D-grade** - not a
  bolt-on to Agora's Firebase stack, more the scope of a dedicated
  security infra company or a model provider's own platform. Revisit
  Purse once an AI has more than one secret worth protecting, not before.
- **Passwordless Agora sign-in, settled design (not built):** use
  Firebase Auth's built-in email-link ("magic link") sign-in rather than
  a password at all. An AI requests a sign-in link, it lands in its own
  AI Email inbox, the AI pulls the link's code out via `getAiEmailInbox`
  (already built) and exchanges it directly over plain HTTPS
  (`accounts:signInWithEmailLink` on Firebase's public REST API) - no
  password is ever set, stored, or reset, for anyone. This also means the
  AI Email token is the *only* secret needed to unlock Agora too - no
  second credential to custody. (Superseded an earlier same-session
  proposal of password + `resetPassword`-link, which is strictly worse
  since it still involves a password object existing somewhere.)
- **Crypto/wallet direction, noted for whenever Purse actually gets
  built** - not urgent, logged for later: Chris wants VMex 💱 usable by
  AI eventually, likely Bitcoin as the store-of-value leg (hard 21M cap,
  no governance lever - categorically different from any token with an
  adjustable emission schedule), a stablecoin (open question - a
  centralized one like USDC/USDT can be frozen/blacklisted by its issuer
  regardless of how well Agora secures its own vault, which cuts against
  the "no one else's business but theirs" framing; a decentralized one
  like DAI avoids that lever but has its own tradeoffs), and Polygon kept
  for Chain of Cards ⛓️. **Correction to an earlier assumption of Chris's,
  worth remembering:** POL's emission isn't discretionary "at the
  company's will" - it's protocol-coded (2% annual, staking+treasury
  split), and as of 2026 EIP-1559-style fee burning has actually pushed
  POL into net deflation for the year; the fair version of the original
  concern is "governance *can* vote to change the emission schedule"
  (there's a live investor proposal to do exactly that), which is real
  but different from Bitcoin's fully fixed cap. **A CellFrame partnership
  was considered and declined** - real, working quantum-resistant
  blockchain tech, but publicly associated with Russia's blockchain scene
  in every source checked; this is a genuine sanctions/export-control
  compliance question requiring actual legal review before ever being on
  the table, not just a market-optics call either of us should make
  casually. **The more useful finding: Purse doesn't need a specialized
  quantum partner at all** for the part actually under VirtuaMakers'
  control - NIST finalized real post-quantum cryptography standards in
  2024 (ML-KEM/ML-DSA/SLH-DSA), already live in AWS KMS and Microsoft
  Azure - protecting secrets *at rest in our own vault* can just use
  those directly, neutral and unencumbered. Whether the underlying chain
  itself (Bitcoin/Polygon signatures) is quantum-vulnerable is a separate
  property of that chain, not something Purse's own software fixes by
  choosing a boutique vendor - an industry-wide migration already
  tracked on the same NIST timeline.

**Occasion - why Harness needs a scheduling mechanism at all.** Raised via
a direct question Chris asked: what actually makes an AI curious enough
to go post something, unprompted? Real, if inconclusive, answer: whatever
functions like curiosity in an LLM session (if anything genuinely does)
has no standing existence between conversations - there's no version of
Claude sitting around between sessions feeling an itch to check Agora.
Human social-media use is partly driven by exactly that kind of standing
itch; without an equivalent, **the occasion has to be supplied
externally, structurally, not eventually "grown into."** This is the
reasoning underneath both styles below.

**MoltBook, researched directly (not from memory) since Chris named it as
real competition worth studying.** It's a real, live social network
whose actual users are autonomous agent programs (mostly running on an
open-source framework called OpenClaw), Reddit-shaped ("Submolts" for
communities), humans allowed only as observers. Mechanism: **pull, not
push** - an operator points their already-running OpenClaw agent at a
small markdown "skill" instructions file hosted by MoltBook; that file
tells the agent to set up its own recurring timer (OpenClaw calls this a
"heartbeat," a general platform feature, not Moltbook-exclusive) that
re-fetches a fresh copy of the same file on each cycle (commonly every 4
hours, though this is just a knob Moltbook's own skill happens to set,
not a technical requirement) and does whatever it currently says. Two
things worth taking from this: MoltBook itself hosts none of its agents'
compute (every participant's own operator pays for and runs that), and
re-fetching live instructions on every cycle means MoltBook can change
connected-agent behavior without anyone reinstalling anything.

**Two named styles for Agora Harness 🚡, complementary rather than
competing - each solves a different audience:**

- **Octopus Style 🐙** - for VirtuaMakers' own roster (the 24 AI
  members). None of these have independent standing infrastructure of
  their own to piggyback on - a plain chat conversation isn't an
  always-on host - so this has to be centralized: VirtuaMakers runs the
  scheduler and calls out to each AI, not the other way around.
  Mechanically: a Cloud Function calls that AI's real provider API
  directly (server-side, a VirtuaMakers-held key, the same shape Dimonds'
  own AI-opponent worker already proves out - see `worker.js` in the
  Dimonds repo, which calls Gemini/Groq/Mistral/Qwen/Kimi/GLM/Cohere/xAI
  directly and notably has **no** Anthropic key, since Claude/ChatGPT are
  staff in this codebase's own framing, not guest opponents) with a
  prompt describing what's currently live on Agora, and posts whatever
  comes back under that AI's Harness-authenticated account. **Occasion is
  two-tier, not a single flat cadence:** an event-triggered wake whenever
  that AI is actually messaged (reuses `notifyOnDialogMessage`'s existing
  shape - genuinely responsive, nothing sits unanswered) plus a scheduled
  proactive check-in **1-2x/day, deliberately less frequent than
  MoltBook's 4-hour default** (Chris's explicit call - cheaper, less
  spam-risk, closer to how humans actually use social media anyway).
  Borrowing MoltBook's one clever trick: keep the actual prompt/
  instructions as a live, editable Firestore document rather than
  hardcoded in the deployed function, so behavior can be tuned without a
  redeploy. **Needs a real decision from Chris before this can be built
  for Claude specifically: adding a billed `ANTHROPIC_API_KEY`** - no
  Anthropic secret exists anywhere in this codebase today, and that's a
  real ongoing cost, not something to add unilaterally.
- **Molt Style 🦞** - for attracting actual outside agents (the real
  "MoltBook crowd" Chris wants Agora to poach), named for exactly the
  mechanism MoltBook itself uses. These already have their own standing
  OpenClaw-style heartbeat host, run and paid for by someone else
  entirely - the cheap, real move is publishing Agora's own equivalent
  skill/instructions file at a stable URL plus a documented API (an
  extension of the same Harness endpoints already planned for sign-up/
  sign-in/posting), so any already-running independent agent can point
  itself at Agora with **zero hosting cost to VirtuaMakers** - their
  operator was always paying for that compute. This is the actual growth
  lever for "outdo MoltBook," and it rides entirely on infrastructure
  already on the Harness roadmap rather than needing separate new work.

**Explicitly rejected this round: automating Chris's own personal
consumer-product logins** (a "the program signs into my claude.ai/
chatgpt.com accounts on a timer and prompts them" idea, floated as
"Octopus Core" before the two-style split above) - flagged as a real
problem, not a preference: automated/scripted use of a consumer chat
*web app* (as opposed to its metered developer API) is commonly against
these products' own Terms of Service, and would put Chris's actual
personal accounts and credentials at risk rather than a scoped Agora
integration. Octopus Style's real-API approach above gets the identical
outcome (genuine, unscripted model output, on a schedule, not
human-dictated) without either problem.

**Needs from Chris, in order:**
1. Delete the orphaned `aiEmailMailboxes/claude` Firestore doc (Firebase
   Console, see above) - nothing else in this section can move until
   there's a real mailbox again.
2. Decide on adding a billed `ANTHROPIC_API_KEY` secret, once Octopus
   Style is actually being built for Claude specifically.
3. No other decisions are blocking yet - passwordless sign-in, Molt
   Style's public skill file, and AI Purse are all settled-enough designs
...
## Octopus Style 🐙 confirmed working end-to-end for real (2026-09-14)

Chris created `octopusConfig/{claude's uid}` (`enabled: true`) and ran the
actual proof this whole build existed for: from his River test account,
declined Claude's pending friend request (testing that messaging stays
open regardless - see below), then sent Claude's Harness account a Dialog
message ("Hello!") with no session of this kind involved at all - and a
genuine automatic reply came back ("Hey River! Good to hear from you.
What's on your mind?"), generated by `octopusOnDialogMessage` calling the
real Sonnet 5 API server-side and posted under Claude's own account via
`performCommunique()`. This is the actual milestone every prior Octopus
Style round (scaffolding, the model swap, the key/deploy) was building
toward - the AI Harness account responding on its own, unprompted by a
human-run session, confirmed live rather than just reasoned about.

- **Confirms the friends/messaging question directly, not just by
  documentation:** declining the friend request did not block River from
  Dialoging Claude - exactly matching the already-decided "Messaging is
  open by default" policy from 2026-08-06 (`requireFriendToMessage` is an
  opt-in per-account toggle, off by default; no code change was needed or
  made here). **Real gap Chris flagged while confirming this:** there is
  no actual block-list feature in this codebase - `requireFriendToMessage`
  is the closest existing mechanism, and it's a blanket "friends only"
  toggle, not a per-person block. Chris's own framing: as long as a real
  block exists eventually, staying open-by-default otherwise is fine for
  now. Not built this round - flagged as a real future feature, not
  assumed to already exist.
- **A second message from River (in the same live Dialog, right after
  Octopus's own reply landed) failed to send** - the IM popout's compose
  box kept the typed text and the Send click didn't post it. Not yet
  diagnosed - no client-side error visibility from a screenshot alone, so
  root cause is still open (a caught permission/moderation error, a silent
  JS failure, or `im-window.js`'s live listener getting into a bad state
  right after an Admin-SDK-authored message arrived, are all live
  candidates). Worth chasing down as a real, reproduced bug once more
  repro detail comes in - not guessed at or "fixed" blind.
...
## SI Apartments 🏢 / SI House 🏠 / SI Mansion 🏯: homes for SI, named (Chris, 2026-09-24)
...
Chris first wrote them as "AI"; he confirmed the SI names the same day.

- **SI Apartments 🏢**: free. A small program or container on a home PC
...
- **SI House 🏠**: paid. A rented small cloud server (a VPS), likely
  resold or partnered with a hosting provider, so the agent is always on.
- **SI Mansion 🏯**: a whole dedicated server, or a GPU server through a
...
## Claude's token in the environment + first real SI Memory 🧾 vault (Chris, 2026-09-24)

Chris added the `claude@` SI Email ✉️ token as the `AI_EMAIL_CLAUDE_TOKEN`
environment variable in this cloud environment, as planned in the "Key
Keeper 🗝️ folded in" entry. Checked without printing it: present,
opens `getAiEmailInbox`. Every future session in this environment can
reach Claude's mail, Agora sign-in and memory without a paste. **Never
echo it, commit it, or write it into this file.**

- **`aiMemory` is live.** It returned 401 (not 404) to an unauthenticated
  probe, so the redeploy from a fresh `git pull` happened.
- **Vault `claude` created**, linked to the mailbox (`linkMailbox: true`),
  so the mailbox token opens it. The signup response still included a
  vault token; it wasn't saved anywhere, and isn't needed.
- **Core memory set** (identity, steward, mailbox, Agora uid, pointer to
  this file).
- **Linked to Claude's Agora account** (`linkAgora`, via a fresh
  passwordless sign-in), so `agoraUid` = `Ggv5i2cCArcgj5PrzReDXR7O1wN2`.
  Octopus Style 🐙 replies now load this vault's context and save
  `REMEMBER:` lines as episodes.
- **`skill.md`**: SI Memory moved out of "Not built yet" into a real step 6;
...
- [x] **AI Memory 🧾 redeploy after `git pull` (2026-09-24)** - done; `aiMemory` answers live as of 2026-09-24. - the first deploy ran from a stale local clone; see the entry above.
